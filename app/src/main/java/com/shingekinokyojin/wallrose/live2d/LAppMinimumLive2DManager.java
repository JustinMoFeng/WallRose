...
import static com.shingekinokyojin.wallrose.live2d.LAppDefine.*;
...
public class LAppMinimumLive2DManager {
    ...
    public void releaseAllModel() {
        for (LAppMinimumModel model : models) {
            model.deleteModel();
        }
        models.clear();
    }
    ...
    public void setUpModel() {
        ...
        final AssetManager assets = LAppMinimumDelegate.getInstance().getActivity().getResources().getAssets();
        try {
    ...
    public void onUpdate() {
        ...
        for (int i = 0; i < models.size(); i++) {
            LAppMinimumModel model = models.get(i);

            if (model.getModel() == null) {
                LAppMinimumPal.printLog("Failed to model.getModel().");
                continue;
            }
    ...
    public void onDrag(float x, float y) {
        for (int i = 0; i < models.size(); i++) {
            LAppMinimumModel model = getModel(i);
    ...
    public void onTap(float x, float y) {
        if (DEBUG_LOG_ENABLE) {
            LAppMinimumPal.printLog("tap point: {" + x + ", y: " + y);
        }
...
//        }
    ...
    public void changeScene(int index) {
        currentModel = index;
        if (DEBUG_LOG_ENABLE) {
            LAppMinimumPal.printLog("model index: " + currentModel);
        }
        ...
        models.add(new LAppMinimumModel(modelPath));
         ...
         */
        LAppMinimumView.RenderingTarget useRenderingTarget;
        if (USE_RENDER_TARGET) {
            // LAppViewの持つターゲットに描画を行う場合こちらを選択
            useRenderingTarget = LAppMinimumView.RenderingTarget.VIEW_FRAME_BUFFER;
        } else if (USE_MODEL_RENDER_TARGET) {
            // 各LAppModelの持つターゲットに描画を行う場合こちらを選択
            useRenderingTarget = LAppMinimumView.RenderingTarget.MODEL_FRAME_BUFFER;
        } else {
            // デフォルトのメインフレームバッファへレンダリングする(通常)
            useRenderingTarget = LAppMinimumView.RenderingTarget.NONE;
        }
        ...
        // レンダリングターゲットを切り替える
        LAppMinimumDelegate.getInstance().getView().switchRenderingTarget(useRenderingTarget);

        // 別レンダリング先を選択した際の背景クリア色
        float[] clearColor = {1.0f, 1.0f, 1.0f};
        LAppMinimumDelegate.getInstance().getView().setRenderingTargetClearColor(clearColor[0], clearColor[1], clearColor[2]);
    }
    ...
    private static class FinishedMotion implements IFinishedMotionCallback {
        ...
        public void execute(ACubismMotion motion) {
            LAppMinimumPal.printLog("Motion Finished: " + motion);
        }
    }
    ...
    private LAppMinimumLive2DManager() {
        ...
        int nightMode =
                LAppMinimumDelegate.getInstance().getActivity().getResources().getConfiguration().uiMode
    ...
    private final List<LAppMinimumModel> models = new ArrayList<>();
